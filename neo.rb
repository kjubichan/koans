...
module Neo
  ...
  class Sensei
    ...
    def show_progress
      ...
      print Color.green("твой путь на данный момент [")
    ...
    def boring_end_screen
      puts "Горы - это не более чем просто горы"
    end
    ...
    def artistic_end_screen
     ...
     ::,:   ,,:::,                                           ,::::::::::::,
    ,:::, :,,:::                                               ::::::::::::,
   ,::: :::::::,       Горы - это не более чем просто горы      ,::::::::::::
   :::,,,::::::                                                   ::::::::::::
 ,:::::::::::,                                                    ::::::::::::,
 :::::::::::,                                                     ,::::::::::::
:::::::::::::                                                     ,::::::::::::
...
ENDTEXT
    ...
    def encourage
      puts
      puts "Хозяин говорит:"
      puts Color.cyan("  Ты еще не достиг просветления.")
      if ((recents = progress.last(5)) && recents.size == 5 && recents.uniq.size == 1)
        puts Color.cyan("  Я чувствую твое расстройство. Не бойся просить о помощи.")
      elsif progress.last(2).size == 2 && progress.last(2).uniq.size == 1
        puts Color.cyan("  Не теряй надежду.")
      elsif progress.last.to_i > 0
        puts Color.cyan("  Ты прогрессируешь. Прекрасно. #{progress.last} выполнено.")
      end
    end

    def guide_through_error
      puts
      puts "Ответы которые ты ищешь..."
      puts Color.red(indent(failure.message).join)
      puts
      puts "Пожалуйста помедитируй над этим кодом:"
    ...
    def a_zenlike_statement
      if !failed?
        zen_statement =  "Горы - это не более чем просто горы"
      else
        zen_statement = case (@pass_count % 10)
        when 0
          "горы - это не более чем просто горы"
        when 1, 2
          "для того чтобы нарушать правила, надо сперва выучить их"
        when 3, 4
          "помни, что иногда молчание – лучший ответ"
        when 5, 6
          "сон – лучшая медитация"
        when 7, 8
          "теряя что-то не теряй приобретенный опыт"
        else
          "вещи не являются тем чем они кажутся, как и тем чем они не кажутся"
        end
      end
